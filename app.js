...
function recommendSkills(job, tierLv, slots) {
  ...
  if (job === "mage") {
    ...
    // 道士：召唤技能全带(每个技能1只宝宝,共存且全部群攻全场,伤害累加) → 隐身术(配合召唤近乎无敌)
    //   → 减伤buff(阴阳盾) → 施毒术 → 单体/被动
    // 数据验证: 3召唤(Σbonus1.6)清波2335 > 1召唤+2AOE的2131, 召唤群攻不扣防且0蓝耗
    ...
    // 减伤 buff（阴阳盾等，隐身术 damageBonus=0 已单独处理）
