...
// 药水候选缓存：AI 选药每回合最多触发2次×数千回合×上百图，
// 不能每次都全量扫描+过滤 ITEMS（这是模拟卡顿的主要来源）
...
function simulateMap(map, player, stats, parts, opts) {
  ...
  const drinkPotion = (kind, cur, max) => {
    ...
    const cands = (kind === "hp" ? getPotionCands().hp : getPotionCands().mp).slice();
    cands.sort((a, b) => Math.abs(a[key] - missing) - Math.abs(b[key] - missing));
...
// 模拟选项：从页面控件读取（选"自动优化"时逐图扫描阈值网格；手动值需与游戏内一致）
...
// 喝药阈值寻优：经验/分最优阈值（并列取净金高、再取阈值高=更安全）
const THRESHOLD_GRID = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2];
const SWEEP_TURNS = 1800; // 寻优扫描回合数（选中最优后再按全长度复算）
const SAFE_DEATHS_PER_HOUR = 30; // 「可挂机」死亡上限：经验方案只在死亡≤30/时的档位里选，
// 否则会推荐"送死流"低阈值（死亡重置波次跳过低经验密度的大血量BOSS，经验反而虚高）

function pickBestSim(sims, metric, maxDeaths) {
  let pool = typeof maxDeaths === "number" ? sims.filter(s => s.deathsPerHour <= maxDeaths) : sims;
...
// 单图计算：自动模式下扫描阈值网格，主值=经验最优阈值；boss图另算boss击杀最优阈值
function calcMapWithSims(map, player, stats, dps, mpPerTurn, parts, simOpts) {
  ...
  if (!simOpts.autoThreshold || r.locked) {
    // 手动阈值 / 等级不足的图：不寻优，直接按当前设置模拟
    r.sim = simulateMap(map, player, stats, parts, {
      ...simOpts, hpThreshold: simOpts.hpThreshold || 0.7,
    });
    r.simSweep = null;
    return r;
  }
  const sims = THRESHOLD_GRID.map(th => simulateMap(map, player, stats, parts, {
    ...simOpts, turns: SWEEP_TURNS, hpThreshold: th,
  }));
  r.simSweep = sims;
  const bestExp = pickBestSim(sims, "expPerMin", SAFE_DEATHS_PER_HOUR);
  r.sim = simulateMap(map, player, stats, parts, { ...simOpts, hpThreshold: bestExp.hpThreshold });
  if (hasBoss) {
    const bestBoss = pickBestSim(sims, "bossKillsPerHour");
    r.bossSim = bestBoss.hpThreshold === bestExp.hpThreshold ? r.sim
      : simulateMap(map, player, stats, parts, { ...simOpts, hpThreshold: bestBoss.hpThreshold });
  }
  return r;
}
...
function renderRow(r, idx) {
  ...
  // 喝药阈值推荐：主值=经验最优；boss图若boss最优阈值不同，追加 🐲标记
  ...
  const potionCell = r.sim
    ? (() => {
        ...
        let html = `<span style="font-weight:600" title="${auto}：HP&lt;${expTh} 喝药（AI自选药水），主列为该阈值下的模拟值">${expTh}</span>`;
        if (r.bossSim && r.bossSim !== r.sim && r.bossSim.bossKillsPerHour > 0)
          html += ` <span style="color:#ffb74d;font-size:11px" title="想多杀BOSS：改用 ${thPct(r.bossSim.hpThreshold)}（boss击杀 ${r.bossSim.bossKillsPerHour}/时 vs ${r.sim.bossKillsPerHour}/时），详见展开行">🐲${thPct(r.bossSim.hpThreshold)}</span>`;
        return html;
      })()
    : '<span style="color:#888">-</span>';
...
// 喝药阈值方案详情：自动寻优给出对比表 + boss图给多杀boss的备选方案
function renderPotionDetail(r) {
  const thPct = th => Math.round(th * 100) + "%";
  if (!r.simSweep) {
    return `<div style="margin-bottom:10px;color:#a5d6ff;">💊 喝药阈值：手动 ${r.sim ? thPct(r.sim.hpThreshold || 0.7) : "70%"}（AI自选药水；切换为「自动优化」可逐图寻优）</div>`;
  }
  const expTh = r.sim.hpThreshold || 0.7;
  const rows = r.simSweep.map(s => {
    const isExp = s.hpThreshold === expTh;
    const isBoss = r.bossSim && r.bossSim !== r.sim && s.hpThreshold === r.bossSim.hpThreshold;
    const mark = isExp ? " 👈经验最优" : (isBoss ? " 🐲boss最优" : "");
    const style = isExp ? "color:#4caf50;font-weight:600" : (isBoss ? "color:#ffb74d" : "");
    return `<tr${style ? ` style="${style}"` : ""}><td>${thPct(s.hpThreshold)}</td><td>${s.deathsPerHour}</td><td>${s.killsPerMin}</td><td>${s.expPerMin.toLocaleString()}</td><td>${s.potGoldPerMin.toLocaleString()}</td><td>${s.netGoldPerMin.toLocaleString()}</td><td>${s.bossKillsPerHour}</td><td>${mark}</td></tr>`;
  }).join("");
  let html = `<div style="margin-bottom:6px;color:#a5d6ff;">💊 喝药阈值寻优（HP阈值扫描 × ${r.simSweep.length}档，AI自选药水，MP阈值固定30%游戏默认）→ 推荐 <b>${thPct(expTh)}</b>（在死亡≤${SAFE_DEATHS_PER_HOUR}/时的"可挂机"档位中取经验最优，避免推荐靠死亡重置波次刷经验的送死流）</div>
  <table class="db-table" style="margin-bottom:10px;font-size:12px;"><thead><tr><th>阈值</th><th>死亡/时</th><th>击杀/分</th><th>经验/分</th><th>药费/分</th><th>净金/分</th><th>boss/时</th><th></th></tr></thead><tbody>${rows}</tbody></table>`;
  if (r.bossSim && r.bossSim !== r.sim && r.bossSim.bossKillsPerHour > 0) {
    const b = r.bossSim, e = r.sim;
    const gain = e.bossKillsPerHour > 0 ? Math.round((b.bossKillsPerHour / e.bossKillsPerHour - 1) * 100) : null;
    html += `<div style="margin-bottom:10px;color:#ffb74d;">🐲 想多杀BOSS：把游戏内 HP喝药阈值 改为 <b>${thPct(b.hpThreshold)}</b> —— boss击杀 ${b.bossKillsPerHour}/时（经验方案 ${thPct(e.hpThreshold)} 为 ${e.bossKillsPerHour}/时${gain !== null ? "，+" + gain + "%" : ""}），死亡 ${e.deathsPerHour}→${b.deathsPerHour}/时；代价：经验 ${e.expPerMin.toLocaleString()}→${b.expPerMin.toLocaleString()}/分、净金 ${e.netGoldPerMin.toLocaleString()}→${b.netGoldPerMin.toLocaleString()}金/分</div>`;
  }
  return html;
}
