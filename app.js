...
function getMpCostPerTurn(player) {
  // 无冷却单体攻击技能耗 2MP/回合 —— 仅法/道（原版第487-502行在法/道魔法段分支内；
  // 战士的 delay=0 攻击技能直接乘物攻、不进魔法段、不耗蓝）
 ...
 * 谁清的波决定宝宝重召：玩家清波=全部重召，宝宝溅射清波=只重召杀手之后的，
 * 主目标死于宝宝=零重召（新波含BOSS时被全额齐射秒杀的根源）。
...
// 药水候选缓存：AI选药每回合都可能调用，不能每次都全量扫描+过滤+排序 ITEMS（模拟卡顿主因）。
// 预按恢复量升序，AI选"最贴合缺口"用线性最近邻+ affordability 双侧游走，零分配。
let _potionCands = null;
function getPotionCands() {
  if (!_potionCands) {
    const hp = [], mp = [];
    for (const info of Object.values(ITEMS)) {
      if (info.type !== "potion") continue;
      if (info.healHp) hp.push(info);        // 含太阳水类（HP优先归红药）
      else if (info.healMp) mp.push(info);
    }
    hp.sort((a, b) => a.healHp - b.healHp);
    mp.sort((a, b) => a.healMp - b.healMp);
    _potionCands = { hp, mp };
  }
  return _potionCands;
}

// AI选药（复刻 _resolveAutoPotion auto 的排序语义：|恢复量-缺口| 升序，并列取原顺序=低档药在前，
// 取第一个买得起的）。list 已按恢复量升序，最近邻向两侧游走等价于该排序。
function aiPickPotion(list, key, missing, gold) {
  let nearest = 0;
  for (let i = 1; i < list.length; i++) {
    if (Math.abs(list[i][key] - missing) < Math.abs(list[nearest][key] - missing)) nearest = i;
  }
  let l = nearest, r = nearest + 1;
  while (l >= 0 || r < list.length) {
    let pick;
    if (l < 0) pick = r++;
    else if (r >= list.length) pick = l--;
    else pick = Math.abs(list[l][key] - missing) <= Math.abs(list[r][key] - missing) ? l-- : r++;
    if (gold >= (list[pick].price || 0)) return list[pick];
  }
  return null;
}
...
function simulateMap(map, player, stats, parts, opts) {
  ...
  // 战士例外：delay=0 攻击技能已并入 warriorMult 乘物攻（原版战士分支不进魔法段、不耗2MP）
  ...
  for (const sk of player.equippedSkills || []) {
    ...
    } else if (player.job !== "warrior" && sd.type === "attack" && !sd.aoe && sk !== "施毒术" && sk !== "瘟疫" && sk !== "毒云") {
  ...
  const cooldowns = {};
  const allDelaySkills = delaySkills.concat(aoeDelay); // 提到循环外，避免每回合分配
  ...
  let curTurn = 0;              // 当前回合数（boss击杀耗时统计用）
  let simGold = player.gold || 1000000; // 金币池：死亡按当前携带金1%扣（无 gold 字段则用100万）
  let kills = 0, bossKills = 0, expGain = 0, deaths = 0, dropGold = 0, potGold = 0;

  // 召唤宝宝模板（原版：atk=SC×bonus×2，hp=max(200, SC×bonus×30)，死后下回合满血重召）
  const minionTemplates = (player.equippedSkills || []).filter(sk => {
    const sd = SKILLS[sk]; return sd && sd.type === "summon";
  }).map(sk => ({
    skillName: sk,
    atk: Math.max(1, Math.floor(scBase * SKILLS[sk].damageBonus * 2.0)),
    maxHp: Math.max(200, Math.floor(scBase * SKILLS[sk].damageBonus * 30)),
  }));
  // 按装备顺序的宝宝攻击力列表（原版按技能迭代顺序逐只攻击）
  const summonMinionAtks = minionTemplates.map(t => t.atk);
  // 记忆戒指/诱惑之光：虚拟宝宝（不建模HP、不死，仅承接仇恨）
  const hasVirtualMinion = parts.hasMinion && minionTemplates.length === 0;
  ...
  const spawnWave = () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (dbg && t.type === "boss") dbg.bossSpawns = (dbg.bossSpawns || 0) + 1;
      monsters.push({ name: chosen, cur: t.hp, maxHp: t.hp, minAtk: t.minAtk, maxAtk: t.maxAtk,
        ...
        exp: t.exp, skills: t.skills || [], stunned: false, spawnTurn: curTurn });
    }
    bossMinions = [];
  };
  ...
  const onKill = (m) => {
    ...
    if (MONSTERS[m.name] && MONSTERS[m.name].type === "boss") {
      bossKills++;
      if (dbg) { dbg.bossKillTurns = (dbg.bossKillTurns || 0) + (curTurn - (m.spawnTurn || 0)); }
    }
  };
  const hit = (m, dmg) => { m.cur -= dmg; if (m.cur <= 0 && !m.dead) { m.dead = true; onKill(m); } };

  // 喝药（复刻 _resolveAutoPotion）：AI自选=恢复量最贴合缺口（付全价、按缺口封顶回复，
  // 缺口89喝100的药就是浪费11血的钱，与游戏一致）；固定档位=只买指定药水（配自然阈值可零浪费）
  const hpPolicy = opts.hpPotion || "auto";
  const mpPolicy = opts.mpPotion || "auto";
  const drinkPotion = (kind, cur, max) => {
    const missing = max - cur;
    const key = kind === "hp" ? "healHp" : "healMp";
    const policy = kind === "hp" ? hpPolicy : mpPolicy;
    const info = policy !== "auto" && ITEMS[policy]
      ? ITEMS[policy]
      : aiPickPotion(kind === "hp" ? getPotionCands().hp : getPotionCands().mp, key, missing, simGold);
    if (!info || simGold < (info.price || 0)) return 0;
    simGold -= info.price || 0; potGold += info.price || 0;
    return Math.min(info[key], missing);
  };
  ...
  const onDeath = () => {
    deaths++;
    if (dbg && monsters.some(m => !m.dead && MONSTERS[m.name] && MONSTERS[m.name].type === "boss"))
      dbg.bossAliveDeaths = (dbg.bossAliveDeaths || 0) + 1;
    ...
    stealthTurns = 0; minions = [];   // 原版死亡清全部状态（隐身/宝宝，下回合重召）
    monsters = []; bossMinions = []; // 波次清零重刷（打了一半的怪作废）
  };

  // 全部存活怪反击一轮（普通回合 / 麻痹回合共用）
  // 原版口径（origin-game.js 682-693）：隐身+有存活宝宝 → 强制打宝宝；
  // 非隐身但有宝宝 → 40%打宝宝；否则打玩家（普攻+技能）。
  // 麻痹回合（origin-game.js 400-417）：所有怪直接围殴玩家（普攻+技能），
  // 完全无视隐身与宝宝仇恨转移。
  const monstersAttack = (stunTurn) => {
    ...
    for (const m of monsters) {
      ...
      let hitsPlayer = stunTurn; // 麻痹回合必打玩家
      if (!hitsPlayer) {
        const divert = stealthTurns > 0
          ? (aliveMinions.length > 0 || hasVirtualMinion)                       // 隐身：有宝宝才转移
      ...
      if (!hitsPlayer) {
        ...
        if (aliveMinions.length > 0) {
          const t = aliveMinions[Math.floor(rng() * aliveMinions.length)];
          t.hp -= Math.max(1, rollDmg(m.minAtk, m.maxAtk, stats.minDef, stats.maxDef));
          if (t.hp <= 0) { t.hp = 0; aliveMinions = aliveMinions.filter(x => x !== t); }
        }
        continue;
      }
    ...
    // BOSS 召唤物攻击（无视隐身/宝宝，不走减伤，扣最小防×0.3）
  ...
  // 波次清除（原版 onMonsterDeath 1048-1076 + 后续阶段）：清全部战斗状态（含隐身/宝宝，
  // 但技能冷却保留 → 隐身需等冷却转好，存在真空期），同 tick 刷新下一波并立即反击。
  // killerIdx 决定重召哪些宝宝（这就是"清波进新波遇BOSS被秒杀"的机制根源）：
  //   -1  = 玩家自己清的波（637行）→ 全部重召并攻击新波（新波回击有40%分流）；
  //    i  = 第i只宝宝的溅射清波（947行）→ 仅重召 i+1 之后的宝宝（不攻击）；
  //   len = 最后一只死于宝宝主目标伤害（641行延后结算）→ 零重召（新波全额齐射玩家）。
  const doWaveClear = (killerIdx) => {
    ...
    minions = (killerIdx < 0 ? minionTemplates : minionTemplates.slice(killerIdx + 1))
      .map(t => ({ ...t, hp: t.maxHp }));
    if (monsters.length === 0) return;
    if (killerIdx < 0) {
      // 原版640行：玩家清波后宝宝阶段照常执行——重召的全部宝宝攻击新波
      for (const a of summonMinionAtks)
        for (const m of monsters) if (!m.dead) hit(m, Math.max(1, Math.floor(a * (0.8 + rng() * 0.4))));
    }
    monstersAttack(); // 新波立即反击（隐身已清零；宝宝=重召子集，可能为0 → 全额齐射+技能）
  ...
  for (let turn = 0; turn < TURNS; turn++) {
    ...
    // 3.1 宝宝重召（原版在玩家行动阶段刷新：死亡的宝宝满血重召）
    minions = minionTemplates.map(t =>
      (minions.find(m => m.skillName === t.skillName && m.hp > 0)) || { ...t, hp: t.maxHp });

    // 技能冷却推进 + 当回合触发加成（复刻 processSkillCooldowns）
    let triggeredBonus = 0; const triggeredAoe = [];
    for (const sk of allDelaySkills) {
      const sd = SKILLS[sk];
      ...
      if (cooldowns[sk] <= 0) {
        ...
        if (mp >= cost) {
          ...
          if (sk === "隐身术" || sk === "集体隐身术") stealthTurns = 6; // 原版：触发即隐身6回合
    ...
    // 5. 施毒/特效（主目标；原版 playerPoisonApply 在宝宝阶段前）
    const main = monsters.find(m => !m.dead);
    if (main) hit(main, parts.poisonSingle + parts.specialSingle);
    // 玩家自己清的波（普攻/魔法/溅射/毒）→ 原版637行结算：宝宝阶段全部重召+攻击新波
    if (monsters.every(m => m.dead)) { doWaveClear(-1); continue; }

    // 5b. 召唤宝宝攻击：按装备顺序逐只结算（roll=atk×0.8~1.2，主目标+全场溅射）。
    // 原版 quirk（935/941/947/641行）：主目标死亡延后到宝宝阶段结束后结算 → 清波时零重召；
    // 溅射死亡即时中途结算 → 清波时仅重召"杀手之后"的宝宝；玩家自己打死的 → 全部重召。
    // 这决定新波首轮回击时有无宝宝承接仇恨——10只波+新波出BOSS时零保护被全额齐射秒杀的根源。
    for (let i = 0; i < summonMinionAtks.length; i++) {
      const m0 = monsters.find(m => !m.dead);
      if (!m0) break; // 主目标已死：本回合后续宝宝不行动（原版935行守卫）
      const a = summonMinionAtks[i];
      m0.cur -= Math.max(1, Math.floor(a * (0.8 + rng() * 0.4))); // 主目标伤害（死亡延后结算）
      if (m0.cur <= 0 && !m0.dead) { m0.dead = true; onKill(m0); if (dbg) dbg.minionMainKills = (dbg.minionMainKills || 0) + 1; }
      let splashed = false;
      for (const m of monsters) { // 溅射其余存活怪（即时结算；原版排除主目标）
        if (m.dead || m === m0) continue;
        splashed = true;
        m.cur -= Math.max(1, Math.floor(a * (0.8 + rng() * 0.4)));
        if (m.cur <= 0 && !m.dead) { m.dead = true; onKill(m); }
      }
      if (monsters.every(m => m.dead)) { doWaveClear(splashed ? i : summonMinionAtks.length); break; }
    }
    if (monsters.every(m => m.dead)) continue; // 已在 doWaveClear 内完成新波回击

    // 6. 怪物反击
    monstersAttack();
    if (hp <= 0) { onDeath(); continue; }

    // 6.5 隐身回合递减（原版在反击后结算；麻痹/毒致死的回合不减）
    if (stealthTurns > 0) stealthTurns--;
  ...
  return {
    ...
    hpThreshold: HP_TH,
    mpThreshold: MP_TH,
    hpPotion: hpPolicy, mpPotion: mpPolicy,
    killsPerMin: Math.round(kills / minutes * 10) / 10,
    bossKillsPerHour: Math.round(bossKills / (TURNS / 3600) * 10) / 10,
...
let calcRunId = 0; // 计算批次代号：重开计算/清空时递增，旧的异步批次检测到后自行终止
...
function loadAndCalc() {
  ...
  // 异步分批计算：蒙特卡洛模拟全图约2秒，同步跑会卡死页面。
  // 每批最多算40ms就让出主线程，算完的地图立刻上表并按当前排序实时重排。
  const simOpts = getSimOptions();
  const runId = ++calcRunId;
  currentResults = [];
  sortKey = "expPerMin"; sortDesc = true; expandedRow = null; // 默认按模拟经验降序
  const progress = document.getElementById("calc-progress");
  document.getElementById("result-panel").style.display = "block";
  renderTable();
  let idx = 0;
  const step = () => {
    if (runId !== calcRunId) return; // 已被新计算/清空取代，本批次作废
    const t0 = Date.now();
    while (idx < MAPS.length) {
      const m = MAPS[idx++];
      const r = calcMapWithSims(m, player, stats, dps, mpPerTurn, parts, simOpts);
      currentResults.push(r);
      if (Date.now() - t0 > 40) break;
    }
    renderTable();
    if (progress) progress.textContent = idx < MAPS.length ? `⏳ 模拟中 ${idx}/${MAPS.length}…` : "";
    if (idx < MAPS.length) setTimeout(step, 0);
  };
  if (progress) progress.textContent = `⏳ 模拟中 0/${MAPS.length}…`;
  setTimeout(step, 0);
}
...
function renderPlayer(player, stats, dps, mpPerTurn, singleAtkSkills) {
  ...
  const mpTxt = mpPerTurn <= 0 ? "0（普攻不耗蓝）"
    : singleAtkSkills > 0 ? `${mpPerTurn} MP（含${singleAtkSkills}个单体技能）`
    : `${mpPerTurn} MP`;
  const cards = [
    ...
    { label: "蓝耗/回合", value: mpTxt, danger: mpPerTurn > 6 },
  ];
...
// 模拟选项：从页面控件读取（选"自动优化"时逐图扫描阈值×药水策略；手动值需与游戏内一致）
function getSimOptions() {
  const hpSel = document.getElementById("sim-hp-threshold");
  const v = hpSel ? hpSel.value : "0.7";
  const potSel = document.getElementById("sim-hp-potion");
  return {
    autoThreshold: v === "auto",
    hpThreshold: v === "auto" ? null : parseFloat(v),
    hpPotion: potSel ? potSel.value : "auto",
    mpThreshold: 0.3,
  };
}
...
const POTION_SHORT = {
  auto: "AI",
  "金创药(小量)": "小量", "金创药(中量)": "中量", "金创药(大量)": "大量",
  "强效金创药": "强效", "金创药(特量)": "特量",
  "魔法药(小量)": "小蓝", "魔法药(中量)": "中蓝", "魔法药(大量)": "大蓝",
  "强效魔法药": "强蓝", "魔法药(特量)": "特蓝",
};

// 喝药策略寻优：
// - AI自选 × 阈值网格8档（AI按缺口选最贴合的药，付全价按缺口封顶，存在过量浪费）
// - 固定药水 × 自然阈值（缺口=药水量时喝，零浪费）+ 安全边际档（更早喝防猝死）
// 自然阈值 T = 1 − 药水量/满血，即 HP<T 时缺口刚超过药水量
const THRESHOLD_GRID = [0.9, 0.8, 0.7, 0.55, 0.4, 0.25];
const HP_POTION_NAMES = ["金创药(小量)", "金创药(中量)", "金创药(大量)", "强效金创药", "金创药(特量)"];
const MP_POTION_NAMES = ["魔法药(小量)", "魔法药(中量)", "魔法药(大量)"];
const SWEEP_TURNS = 1200; // 寻优扫描回合数（选中最优后再按全长度复算）
const SAFE_DEATHS_PER_HOUR = 30; // 「可挂机」死亡上限：经验/boss方案都在死亡≤30/时的档位里选，
// 否则会推荐"送死流"低阈值（死亡重置波次跳过低经验密度的大血量BOSS或反复重rollboss，指标虚高）

const clampTh = t => Math.max(0.05, Math.min(0.95, Math.round(t * 100) / 100));

// 确定性随机（每图固定种子）：同一图每次计算结果一致；且同图各策略组合共用同一条
// 随机流（同波次构成/同伤害序列，只差喝药策略），显著降低策略间对比方差
...
function mapSeed(map) {
  let h = 0;
  const s = String(map.id || map.name);
  for (let i = 0; i < s.length; i++) h = (Math.imul(h, 31) + s.charCodeAt(i)) | 0;
  return (h ^ 0x9E3779B9) >>> 0;
}

function buildHpCombos(stats) {
  const combos = THRESHOLD_GRID.map(th => ({ th, potion: "auto" }));
  for (const name of HP_POTION_NAMES) {
    const heal = ITEMS[name].healHp;
    if (heal > stats.maxHp * 0.9) continue; // 药水量接近/超过满血：钳到5%阈值=送死，不纳入
    combos.push({ th: clampTh(1 - heal / stats.maxHp), potion: name }); // 零浪费档
  }
  return combos;
}

function buildMpCombos(stats) {
  const combos = [{ th: 0.3, potion: "auto" }]; // 游戏默认
  for (const name of MP_POTION_NAMES) {
    const heal = ITEMS[name].healMp;
    if (heal > stats.maxMp * 0.9) continue;
    combos.push({ th: clampTh(1 - heal / stats.maxMp), potion: name });
  }
  return combos;
}

function pickBestSim(sims, metric, maxDeaths) {
  let pool = sims;
  if (typeof maxDeaths === "number") {
    pool = sims.filter(s => s.deathsPerHour <= maxDeaths);
    if (pool.length === 0) {
      // 全部超过死亡上限：放宽到最低死亡档的1.5倍以内再取最优（避免推荐极端送死流）
      const minD = Math.min(...sims.map(s => s.deathsPerHour));
      pool = sims.filter(s => s.deathsPerHour <= minD * 1.5 + 1e-9);
      if (pool.length === 0) pool = sims;
    }
  }
  let best = null;
  for (const s of pool) {
    if (!best) { best = s; continue; }
    if (s[metric] > best[metric]) best = s;
    else if (s[metric] === best[metric] &&
      (s.netGoldPerMin > best.netGoldPerMin ||
        (s.netGoldPerMin === best.netGoldPerMin && s.hpThreshold > best.hpThreshold))) best = s;
  }
  return best;
}

// 单图计算：自动模式下扫描 红药策略(阈值×药水) → 蓝药策略 → 满长度复算；boss图另算boss最优策略
function calcMapWithSims(map, player, stats, dps, mpPerTurn, parts, simOpts) {
  const r = calcMap(map, player, stats, dps, mpPerTurn, parts);
  const hasBoss = map.monsters.some(mm => MONSTERS[mm.name] && MONSTERS[mm.name].type === "boss");
  const seeded = extra => ({ rng: mulberry32(mapSeed(map)), ...simOpts, ...extra });
  if (!simOpts.autoThreshold || r.locked) {
    // 手动策略 / 等级不足的图：不寻优，直接按当前设置模拟
    r.sim = simulateMap(map, player, stats, parts, seeded({
      hpThreshold: simOpts.hpThreshold || 0.7,
    }));
    r.simSweep = null;
    return r;
  }
  // 1) 红药策略扫描（蓝药先按游戏默认 AI@30%）
  const hpSims = buildHpCombos(stats).map(c => simulateMap(map, player, stats, parts, seeded({
    turns: SWEEP_TURNS, hpThreshold: c.th, hpPotion: c.potion,
  })));
  const bestHp = pickBestSim(hpSims, "expPerMin", SAFE_DEATHS_PER_HOUR);
  // 2) 蓝药策略：固定最优红药下扫描（按净金选，需明显更优才替换默认AI@30%，防噪声翻转）；
  //    无蓝耗职业（无触发技能且解析蓝耗为0）跳过
  const mpCombos = buildMpCombos(stats);
  const mpSims = mpPerTurn <= 0 && !(player.equippedSkills || []).some(sk => SKILLS[sk] && SKILLS[sk].delay > 0)
    ? [{ ...bestHp }] // 占位：沿用红药最优那次扫描结果对应的默认蓝药
    : mpCombos.map(c => simulateMap(map, player, stats, parts, seeded({
        turns: SWEEP_TURNS,
        hpThreshold: bestHp.hpThreshold, hpPotion: bestHp.hpPotion,
        mpThreshold: c.th, mpPotion: c.potion,
      })));
  let bestMp = mpSims[0];
  for (const s of mpSims.slice(1)) if (s.netGoldPerMin > bestMp.netGoldPerMin * 1.02) bestMp = s;
  // 3) 满长度复算主方案（boss图加长到6000回合：boss击杀是稀有事件，短样本的boss/时噪声可达±35%）
  const FINAL_TURNS = hasBoss ? 6000 : SIM_DEFAULT_TURNS;
  r.sim = simulateMap(map, player, stats, parts, seeded({
    turns: FINAL_TURNS,
    hpThreshold: bestHp.hpThreshold, hpPotion: bestHp.hpPotion,
    mpThreshold: bestMp.mpThreshold, mpPotion: bestMp.mpPotion,
  }));
  r.simSweep = hpSims; r.simSweepMp = mpSims;
  if (hasBoss) {
    const bestBoss = pickBestSim(hpSims, "bossKillsPerHour", SAFE_DEATHS_PER_HOUR);
    const same = bestBoss.hpThreshold === bestHp.hpThreshold && bestBoss.hpPotion === bestHp.hpPotion;
    if (!same) {
      const bossSim = simulateMap(map, player, stats, parts, seeded({
        turns: FINAL_TURNS,
        hpThreshold: bestBoss.hpThreshold, hpPotion: bestBoss.hpPotion,
        mpThreshold: bestMp.mpThreshold, mpPotion: bestMp.mpPotion,
      }));
      // 终值校验：boss方案需在满长度复算中比经验方案多杀≥10%的boss才展示（防噪声/微小提升不值得换死亡）
      if (bossSim.bossKillsPerHour >= r.sim.bossKillsPerHour * 1.1) r.bossSim = bossSim;
    }
    if (!r.bossSim) r.bossSim = r.sim;
  }
  return r;
}
...
const COLUMNS = [
  ...
  { key: "potion",   label: "喝药🎯",      sort: null,                       align: "center" },
...
function renderRow(r, idx) {
  ...
  // 喝药策略推荐：主值=经验最优（阈值+药水档位）；boss图若boss最优策略不同，追加 🐲标记
  const thPct = th => Math.round(th * 100) + "%";
  const potionCell = r.sim
    ? (() => {
        const expTh = thPct(r.sim.hpThreshold || 0.7);
        const expPot = POTION_SHORT[r.sim.hpPotion] || "AI";
        const auto = r.simSweep ? "自动寻优" : "手动设置";
        let html = `<span style="font-weight:600" title="${auto}：HP&lt;${expTh} 喝${expPot === "AI" ? "药（AI自选）" : "红药(" + expPot + ")"}，主列为该策略下的模拟值">${expTh}·${expPot}</span>`;
        if (r.bossSim && r.bossSim !== r.sim && r.bossSim.bossKillsPerHour > 0)
          html += ` <span style="color:#ffb74d;font-size:11px" title="想多杀BOSS：改用 ${thPct(r.bossSim.hpThreshold)}·${POTION_SHORT[r.bossSim.hpPotion] || "AI"}（boss击杀 ${r.bossSim.bossKillsPerHour}/时 vs ${r.sim.bossKillsPerHour}/时），详见展开行">🐲${thPct(r.bossSim.hpThreshold)}·${POTION_SHORT[r.bossSim.hpPotion] || "AI"}</span>`;
        return html;
      })()
    : '<span style="color:#888">-</span>';
  ...
  const row = `<tr class="${rowCls}" id="row-${idx}">
    ...
    <td class="num">${potionCell}</td>
    <td class="drops-cell">${dropHtml}</td>
  </tr>`;

  const detailRow = expandedRow === idx
    ? `<tr class="detail-row"><td colspan="10">${renderDetail(r)}</td></tr>` : "";
...
function renderDetail(r) {
  ...
  return danger + simInfo + potionInfo + lethalInfo + goldInfo + mpInfo + hpInfo + drops;
}

// 喝药策略详情：自动寻优给出阈值×药水对比表 + 蓝药建议 + boss图备选方案
function renderPotionDetail(r) {
  const thPct = th => Math.round(th * 100) + "%";
  const pot = p => POTION_SHORT[p] || "AI";
  if (!r.simSweep) {
    return `<div style="margin-bottom:10px;color:#a5d6ff;">💊 喝药策略：手动 ${r.sim ? thPct(r.sim.hpThreshold || 0.7) + "·" + pot(r.sim.hpPotion) : "70%·AI"}（切换为「自动优化」可逐图寻优阈值+药水档位）</div>`;
  }
  const expTh = r.sim.hpThreshold || 0.7, expPot = r.sim.hpPotion || "auto";
  const rows = r.simSweep.map(s => {
    const isExp = s.hpThreshold === expTh && s.hpPotion === expPot;
    const isBoss = r.bossSim && r.bossSim !== r.sim && s.hpThreshold === r.bossSim.hpThreshold && s.hpPotion === r.bossSim.hpPotion;
    const mark = isExp ? " 👈经验最优" : (isBoss ? " 🐲boss最优" : "");
    const style = isExp ? "color:#4caf50;font-weight:600" : (isBoss ? "color:#ffb74d" : "");
    return `<tr${style ? ` style="${style}"` : ""}><td>${thPct(s.hpThreshold)}</td><td>${pot(s.hpPotion)}</td><td>${s.deathsPerHour}</td><td>${s.killsPerMin}</td><td>${s.expPerMin.toLocaleString()}</td><td>${s.potGoldPerMin.toLocaleString()}</td><td>${s.netGoldPerMin.toLocaleString()}</td><td>${s.bossKillsPerHour}</td><td>${mark}</td></tr>`;
  }).join("");
  let html = `<div style="margin-bottom:6px;color:#a5d6ff;">💊 红药策略寻优（阈值×药水档位 × ${r.simSweep.length}组合，扫描${SWEEP_TURNS}回合；固定药水配"自然阈值"=缺口等于药水量时喝，零过量浪费）→ 推荐 <b>${thPct(expTh)}·${pot(expPot)}</b>（在死亡≤${SAFE_DEATHS_PER_HOUR}/时的"可挂机"档位中取经验最优）</div>
  <table class="db-table" style="margin-bottom:6px;font-size:12px;"><thead><tr><th>阈值</th><th>红药</th><th>死亡/时</th><th>击杀/分</th><th>经验/分</th><th>药费/分</th><th>净金/分</th><th>boss/时</th><th></th></tr></thead><tbody>${rows}</tbody></table>
  <div style="margin-bottom:6px;color:#666;font-size:11px;">注：扫描为短样本（${SWEEP_TURNS}回合），boss/时 列噪声较大（±30%+），以主方案的满长度模拟为准。</div>`;
  // 蓝药建议（净金口径，需比默认 AI@30% 明显更优才换）
  const m = r.sim;
  const mpTxt = `💧 蓝药建议：${thPct(m.mpThreshold || 0.3)}·${pot(m.mpPotion)}（净金 ${m.netGoldPerMin.toLocaleString()}/分${r.simSweepMp && r.simSweepMp[0] ? "，vs 默认AI@30% " + r.simSweepMp[0].netGoldPerMin.toLocaleString() + "/分" : ""}）`;
  html += `<div style="margin-bottom:10px;color:#90caf9;">${mpTxt}</div>`;
  if (r.bossSim && r.bossSim !== r.sim && r.bossSim.bossKillsPerHour > 0) {
    const b = r.bossSim, e = r.sim;
    const gain = e.bossKillsPerHour > 0 ? Math.round((b.bossKillsPerHour / e.bossKillsPerHour - 1) * 100) : null;
    html += `<div style="margin-bottom:10px;color:#ffb74d;">🐲 想多杀BOSS：把游戏内喝药改为 <b>${thPct(b.hpThreshold)}·${pot(b.hpPotion)}</b> —— boss击杀 ${b.bossKillsPerHour}/时（经验方案为 ${e.bossKillsPerHour}/时${gain !== null ? "，+" + gain + "%" : ""}），死亡 ${e.deathsPerHour}→${b.deathsPerHour}/时；代价：经验 ${e.expPerMin.toLocaleString()}→${b.expPerMin.toLocaleString()}/分、净金 ${e.netGoldPerMin.toLocaleString()}→${b.netGoldPerMin.toLocaleString()}金/分</div>`;
  }
  return html;
}
