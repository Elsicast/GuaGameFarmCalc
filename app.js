...
function simulateMap(map, player, stats, parts, opts) {
  ...
  }).map(sk => ({ skillName: sk, maxHp: Math.max(200, Math.floor(scBase * SKILLS[sk].damageBonus * 30)) }));
...
function calcMapWithSims(map, player, stats, dps, mpPerTurn, parts, simOpts) {
  ...
  if (hasBoss) {
    ...
    if (!same) {
      ...
      // 终值校验：boss方案必须在满长度复算中严格优于经验方案才展示（防扫描噪声翻转）
      if (bossSim.bossKillsPerHour > r.sim.bossKillsPerHour) r.bossSim = bossSim;
    }
